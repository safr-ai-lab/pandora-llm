...
def plot_ROC_multiple_ci(train_statistics_list, val_statistics_list, plot_title, labels, keep_first=None, show_plot=True, save_name=None, log_scale=False, fprs=None, colors=None):
    '''
    Plots multiple ROC curves in a single plot
    ...
    for i, (train_statistics, val_statistics, label) in enumerate(zip(train_statistics_list, val_statistics_list,labels)):
        ...
        bootstrap_result = bootstrap((data,), auc_statistic, confidence_level=0.95, n_resamples=100, batch=1, method='percentile',axis=0)
