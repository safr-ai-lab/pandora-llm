...
from pandora_llm.attacks.MinK import MinK
...
def main():
    ...
    if args.experiment_name is None:
        args.experiment_name = (
            (f"MinK_{args.model_name.replace('/','-')}") +
        ...
        args.experiment_name = f"results/MinK/{args.experiment_name}/{args.experiment_name}"
    ...
    # Compute statistics
    MinKer.load_model()
    train_statistics_tokens = MinKer.compute_statistic_tokens(training_dataloader,num_batches=math.ceil(args.num_samples/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    torch.save(train_statistics_tokens,f"{args.experiment_name}_train.pt")
    val_statistics_tokens = MinKer.compute_statistic_tokens(validation_dataloader,num_batches=math.ceil(args.num_samples/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    ...
    MinKer.attack_plot_histogram(train_statistics_tokens, val_statistics_tokens, title=args.experiment_name, normalize=False, show_plot=False)
    MinKer.attack_plot_histogram(train_statistics_tokens, val_statistics_tokens, title=args.experiment_name, normalize=True, show_plot=False)
