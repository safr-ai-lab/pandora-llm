...
def main():
    ...
    parser.add_argument('--ft_model_name', action="store", type=str, required=True, help='Huggingface model name')
    parser.add_argument('--ft_model_revision', action="store", type=str, required=False, help='Model revision. If not specified, uses main.')
    parser.add_argument('--ft_model_cache_dir', action="store", type=str, required=False, help='Model cache directory. If not specified, uses main.')
    # Dataset Arguments
    ...
    # Initialize attack
    FLoRaer = FLoRa(args.model_name, args.ft_model_name, model_revision=args.model_revision, model_cache_dir=args.model_cache_dir, ft_model_revision=args.ft_model_revision, ft_model_cache_dir=args.ft_model_cache_dir)
    
    # Compute statistics
    FLoRaer.load_model("base")
    ground_truth_statistics_base = FLoRaer.compute_statistic(ground_truth_dataloader,num_batches=math.ceil(args.num_samples/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    generations_statistics_base = FLoRaer.compute_statistic(generations_dataloader,num_batches=math.ceil(args.num_samples*generations.shape[1]/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    generations_statistics_base = generations_statistics_base.reshape(generations.shape[0],generations.shape[1])
    FLoRaer.unload_model()
    FLoRaer.load_model("ft")
    ground_truth_statistics_ft = FLoRaer.compute_statistic(ground_truth_dataloader,num_batches=math.ceil(args.num_samples/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    generations_statistics_ft = FLoRaer.compute_statistic(generations_dataloader,num_batches=math.ceil(args.num_samples*generations.shape[1]/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    generations_statistics_ft = generations_statistics_ft.reshape(generations.shape[0],generations.shape[1])
    FLoRaer.unload_model()
    ground_truth_statistics = ground_truth_statistics_ft/ground_truth_statistics_base
    generations_statistics = generations_statistics_ft/generations_statistics_base
