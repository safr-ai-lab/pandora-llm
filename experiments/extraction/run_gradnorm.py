...
def main():
    ...
    ground_truth_statistics = GradNormer.compute_gradients(ground_truth_dataloader,norms=args.norms,num_batches=math.ceil(args.num_samples/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    torch.save(ground_truth_statistics,f"{args.experiment_name}_true_statistics.pt")
    generations_statistics = GradNormer.compute_gradients(generations_dataloader,norms=args.norms,num_batches=math.ceil(args.num_samples*generations.shape[1]/args.bs),device=device,model_half=args.model_half,accelerator=accelerator)
    for feature in generations_statistics:
        print(feature)
        print(generations_statistics[feature].shape)
        generations_statistics[feature] = generations_statistics[feature].reshape(generations.shape[0],generations.shape[1],*generations_statistics[feature].shape[1:])
    torch.save(generations_statistics,f"{args.experiment_name}_gen_statistics.pt")
    GradNormer.unload_model()

    # Compute metrics
    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    for feature in generations_statistics.keys():
        if generations_statistics[feature].ndim==1:
            compute_extraction_metrics(
                ground_truth=ground_truth,
                generations=generations,
                ground_truth_statistics=ground_truth_statistics[feature],
                generations_statistics=generations_statistics[feature],
                ground_truth_probabilities=ground_truth_probabilities,
                prefix_length=args.prefix_length,
                suffix_length=args.suffix_length,
                tokenizer=tokenizer,
                title=args.experiment_name+f"_{feature}",
                statistic_name=f"GradNorm_{feature}",
            )
