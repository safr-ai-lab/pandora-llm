...
python run_loss.py --mod_size 70m --deduped --checkpoint step98000 --n_samples 1000 --pack

Sample command line prompt (with acceleration)
accelerate launch run_loss.py --accelerate --mod_size 70m --deduped --checkpoint step98000 --n_samples 100 --pack
"""
...
def main():
    ...
    if not (args.pack ^ args.unpack):
        if accelerator is None or accelerator.is_main_process: # so only print on main thread; otherwise prints N times for N GPUs
